...
# ---------------------------------------------------------------------------
# Starting workflow: Clean Agent
# ---------------------------------------------------------------------------
log INFO 'Starting workflow: Clean Agent'

# ---------------------------------------------------------------------------
# Step 1 (vars): Read heading
# ---------------------------------------------------------------------------
step_read_heading() {
  local status=0
...
run_stateful_step step_read_heading

# ---------------------------------------------------------------------------
# Step 2 (agent): Ask custom agent
# ---------------------------------------------------------------------------
step_ask_custom_agent() {
  local prompt
  prompt=$(cat <<PROMPT_EOF
Your name is test. The repository heading is ${README_HEADING}.
Reply in under 20 words. Do not modify files.
PROMPT_EOF
  )
  local agent='test'
  run_agent "$prompt" "$agent"
}
run_step step_ask_custom_agent

# ---------------------------------------------------------------------------
# Workflow finished: Clean Agent
# ---------------------------------------------------------------------------
log INFO 'Workflow finished: Clean Agent'
