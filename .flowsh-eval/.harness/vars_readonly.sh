...
# ---------------------------------------------------------------------------
# Starting workflow: Vars Readonly
# ---------------------------------------------------------------------------
log INFO 'Starting workflow: Vars Readonly'

# ---------------------------------------------------------------------------
# Step 1 (vars): Collect repo facts
# ---------------------------------------------------------------------------
step_collect_repo_facts() {
  local status=0
  CURRENT_BRANCH=$(bash -euo pipefail <<'VARS_CURRENT_BRANCH_EOF'
git branch --show-current
VARS_CURRENT_BRANCH_EOF
  )
  status=$?
  if [[ $status -ne 0 ]]; then
    return "$status"
  ...
  FILE_COUNT=$(bash -euo pipefail <<'VARS_FILE_COUNT_EOF'
...
VARS_FILE_COUNT_EOF
  ...
  export FILE_COUNT
  README_TITLE=$(bash -euo pipefail <<'VARS_README_TITLE_EOF'
sed -n '1p' README.md
VARS_README_TITLE_EOF
  )
  status=$?
  if [[ $status -ne 0 ]]; then
    return "$status"
...
run_stateful_step step_collect_repo_facts

# ---------------------------------------------------------------------------
# Step 2 (bash): Print repo facts
# ---------------------------------------------------------------------------
step_print_repo_facts() {
...
run_step step_print_repo_facts

# ---------------------------------------------------------------------------
# Workflow finished: Vars Readonly
# ---------------------------------------------------------------------------
log INFO 'Workflow finished: Vars Readonly'
